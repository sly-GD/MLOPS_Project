...
"""
============================================
监督微调（Supervised Fine-Tuning）主循环
============================================
与预训练的核心区别：
  1. 使用 SFTDataset — 对话格式数据，loss 只计算 assistant 回复部分
  2. 学习率更小（1e-5 vs 5e-4），训练更稳定
  3. 一般基于预训练权重继续训练
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=1e-5, help="初始学习率（AdamW组）")
    ...
    """
    SFT 数据关键差异（vs 预训练）：
    1. SFTDataset 加载的是多轮对话 jsonl，包含 role/content 字段
    2. 每条样本会经过 apply_chat_template 格式化为完整 prompt
    3. generate_labels() 只标记 assistant 回复部分的 token 为有效 label
       （user/系统消息对应的位置设为 -100，不参与 loss 计算）
    4. 默认基于预训练权重（from_weight='pretrain'）继续训练
    5. 学习率从 5e-4 降到 1e-5，防止破坏预训练学到的知识
    """
