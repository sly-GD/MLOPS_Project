    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        for param_group in optimizer.param_groups:
            if param_group.get('use_muon', False):
                param_group['lr'] = optimizer._muon_base_lr * (lr / args.learning_rate)
            else:
                param_group['lr'] = optimizer._adam_base_lr * (lr / args.learning_rate)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=5e-6, help="初始学习率（AdamW组）")
